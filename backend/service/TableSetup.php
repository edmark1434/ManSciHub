...
class TableSetup {
    ...
    public function createTablesIfNotExist() {
        try {
            $stmt = $this->pdo->query("SELECT to_regclass('public.student')");
            ...
            if ($studentTable === null) {
                ...
                echo json_encode(["message" => "Tables already exist. No action taken."]);
            }

        } catch (PDOException $e) {
            http_response_code(500);
            echo json_encode(["message" => "Error creating tables: " . $e->getMessage()]);
        }
    }
}
?>
